...
## Financial Services Track: Schemes Eligibility Checker

This project includes a fully functional voice-based **Government Schemes Eligibility Checker** under the Financial Services track. 
...
### Core Features:
- **Remote API Sync & Fallback:** The agent queries a remote URL for the latest eligibility criteria. If the network call times out (2.0s limit) or fails, it falls back to a hand-built local dataset (`backend/src/schemes_data.json`) updated on **August 10, 2026**.
- **Dialogue Awareness:** The agent mentions the date of the rules version during the conversation. If it runs in offline fallback mode, it informs the caller: *"I couldn't reach the live government portal, so I am using cached rules updated on August 10, 2026."*
- **Step-by-step Assessment & Checklists:** The agent collects user metrics (age, income, unorganized worker status) one-by-step and produces the final eligibility status with a document checklist.
